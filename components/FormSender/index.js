...
export default class FormSender extends HTMLElement {
    ...
    applyFormInteractionListeners() {
        FormSender.formInteractionEvents.forEach((event) => {
            this.form.addEventListener(event, this.formInteractionHandler, {
                once: true,
            });
        });
    }
    ...
    clearFormInteractionListeners() {
        FormSender.formInteractionEvents.forEach((event) => {
            this.form.removeEventListener(event, this.formInteractionHandler, {
                once: true,
            });
        });
    }
    ...
    setStatusColor(color) {
        this.ajaxStatusEl.classList.remove(
            STATUS_COLOR_NORMAL,
            STATUS_COLOR_SUCCESS,
            STATUS_COLOR_ERROR
        );
